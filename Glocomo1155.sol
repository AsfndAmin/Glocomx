...
contract MyToken is ERC1155, AccessControl {
    ...
    constructor(address _ownerAddress, address _companyAddress, uint256[] memory _supply, uint256[] memory _price) ERC1155("") {   
       
        require(_ownerAddress != address(0) && _companyAddress!= address(0) , "address cant be 0");
        ...
        
    {
        ...
        IERC20(paymentToken[paymentIndx]).transferFrom(msg.sender, companyAddress, price);
        _mint(account, id, amount, "");
        _totalSupply[id] += amount;
    }

    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, uint16 paymentIndx)
        public
       
    {
        uint256 totalPrice;
        for (uint i = 0; i < ids.length; i++) { 
             require(_tokens[ids[i]].maxSupply != 0, "not available");
             require(_totalSupply[ids[i]] + amounts[i] <= _tokens[ids[i]].maxSupply, "maxSupply Reched"); 
            totalPrice =  totalPrice + (_tokens[ids[i]].mintPrice*amounts[i]);
            _totalSupply[ids[i]] += amounts[i];
        }
        IERC20(paymentToken[paymentIndx]).transferFrom(msg.sender, companyAddress, totalPrice);
